package tools;

import javafx.application.Application;
import javafx.embed.swing.SwingFXUtils;
import javafx.fxml.FXMLLoader;
import javafx.scene.Parent;
import javafx.scene.Scene;
import javafx.stage.Stage;
import javafx.scene.image.WritableImage;

import javax.imageio.ImageIO;
import java.io.File;
import java.nio.file.*;
import java.util.List;
import java.util.stream.Collectors;

public class SimpleFXMLScreenshotter extends Application {
    @Override
    public void start(Stage stage) throws Exception {
        System.out.println("Starting FXML Screenshotter...");
        
        Path fxmlDir = Paths.get("src/main/resources/fxml");
        Path outDir  = Paths.get("screenshots");
        Files.createDirectories(outDir);

        System.out.println("Scanning for FXML files in: " + fxmlDir.toAbsolutePath());

        List<Path> fxmlFiles = Files.walk(fxmlDir)
                .filter(p -> p.toString().endsWith(".fxml"))
                .collect(Collectors.toList());

        System.out.println("Found " + fxmlFiles.size() + " FXML files");

        for (Path fxml : fxmlFiles) {
            try {
                System.out.println("Processing: " + fxml.getFileName());
                
                Parent root = FXMLLoader.load(fxml.toUri().toURL());
                Scene scene = new Scene(root);
                stage.setScene(scene);
                stage.sizeToScene();
                stage.show(); // needs to show before snapshot

                // Give the scene time to render
                Thread.sleep(100);

                WritableImage img = scene.snapshot(null);
                String name = fxml.getFileName().toString().replace(".fxml", ".png");
                File out = outDir.resolve(name).toFile();
                ImageIO.write(SwingFXUtils.fromFXImage(img, null), "png", out);

                System.out.println("Saved: " + out.getAbsolutePath());
                
                // Hide the stage for next iteration
                stage.hide();
                
            } catch (Exception e) {
                System.err.println("Error processing " + fxml.getFileName() + ": " + e.getMessage());
                e.printStackTrace();
            }
        }
        
        System.out.println("Screenshot generation complete!");
        stage.close();
        System.exit(0);
    }

    public static void main(String[] args) {
        launch(args);
    }
} 